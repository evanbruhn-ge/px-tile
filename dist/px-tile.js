 ...
 */(function(){Polymer({is:'px-tile',properties:{/**
       ...
       */mainTitle:{type:String,value:'',observer:'_onTitleChanged'},/**
       ...
       */titleEditable:{type:String,value:'',observer:'_titleEditableChanged'},/**
       ...
       */titleValidator:{type:Function,observer:'_titleValidatorChanged'},/**
       * Custom function that's invoked when the drop event is fired
       */dropHandler:{type:Function,observer:'_dropHandlerChanged'},/**
       * Property used internally for showing or hiding the edit title form
       */_showEditForm:{type:Boolean,value:true},/**
       * Property used internally for determining the selected state on the px-tile
       */_selected:{type:Boolean,value:false,observer:'_selectedChanged'}},/**
     ...
     */_handleDrag:function _handleDrag(e){e.preventDefault();this._selected=e.type==='dragenter'},/**
     ...
     */_handleDrop:function _handleDrop(e){e.preventDefault();if(e.type==='drop'){this._selected=false;if(typeof this.dropHandler==='function')this.dropHandler(e.dataTransfer.getData('text'),this,e)}},/**
     * Sets or removes the selected class from the px-tile container based on the selected state
     */_selectedChanged:function _selectedChanged(){if(this._selected)this.$.tile.classList.add('selected');else this.$.tile.classList.remove('selected')},/**
...
this._hoverTextColor=window.getComputedStyle(this.$.overlay).color}},/**
     ...
     */attached:function attached(){this.listen(this.$.overlay,'mouseenter','_hover');this.listen(this.$.overlay,'mouseleave','_hover');this.listen(this,'dragenter','_handleDrag');this.listen(this,'dragleave','_handleDrag');this.listen(this,'dragover','_handleDrop');this.listen(this,'drop','_handleDrop')},/**
     * Detach event listeners for hoverable tiles.
     */detached:function detached(){this.unlisten(this.$.overlay,'mouseenter');this.unlisten(this.$.overlay,'mouseleave');this.unlisten(this,'dragenter');this.unlisten(this,'dragleave');this.listen(this,'dragover');this.listen(this,'drop')},/**
     ...
     */_actionButtonsChanged:function _actionButtonsChanged(){this._hasPrimaryBtn=false;this._hasActionButtons=this.actionButtons&&this.actionButtons.items&&this.actionButtons.items.length>0;if(this._hasActionButtons){var maxPrimaryButtons=this.actionButtons.maxPrimaryButtons||1;for(var x in this.actionButtons.items){if(this.actionButtons.items[x].isPrimary){this._hasPrimaryBtn=true;break}}}this._onDataChanged()},_onTitleChanged:function _onTitleChanged(newTitle,oldTitle){this._onDataChanged();// Do not fire event if the old title is undefined (to avoid firing the event when the element is first created)
if(oldTitle===undefined)return;// Fire the event with the new title, and the previous title if it was set.
var eventDetail={title:newTitle};if(oldTitle)eventDetail.previousTitle=oldTitle;this.fire('px-tile-title-changed',eventDetail)},/**
     * On change callback for either property to set _hasTitleActionBtn, _hasTitleSubtitleActionBtn, and _hasData flags
     */_onDataChanged:function _onDataChanged(){this._hasTitleActionBtn=this.mainTitle&&this.mainTitle.length>0||this._hasPrimaryBtn;this._hasTitleSubtitleActionBtn=this.subtitle&&this.subtitle.length>0||this._hasTitleActionBtn;this._hasData=this._hasActionButtons||this._hasTitleSubtitleActionBtn;this._showEditForm=this.mainTitle.length===0},/**
     ...
     */_titleEditableChanged:function _titleEditableChanged(title){// TODO: This does nothing yet.
},/**
     * Observer for title-validator property changes.
     */_titleValidatorChanged:function _titleValidatorChanged(validator){// Handle functions specified in component markup (ie. strings)
if(typeof validator==='string'){// TODO: There must be a better way!
this.titleValidator=eval(validator);return}},/**
     * Observer for drop-handler property changes.
     */_dropHandlerChanged:function _dropHandlerChanged(dropHandler){// Handle functions specified in component markup (ie. strings)
if(typeof dropHandler==='string'){// TODO: There must be a better way!
this.dropHandler=eval(dropHandler);return}}/**
